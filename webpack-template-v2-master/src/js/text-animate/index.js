...
for (var i = 0; i < titles.length; i++) {
    if (scrollTop > titles[i].breakpoint && scrollTop < (titles[i].breakpoint + windowHeight)) {
...
setTimeout(function () {
    ...
    for (var i = 0; i < current.length; i++) {
        if (current[i] != final[i]) {
            current = correct + possible.charAt(Math.floor(Math.random() * possible.length)) + current.slice(i + 1);
            break;
        } else {
            correct += current[i];
        }
